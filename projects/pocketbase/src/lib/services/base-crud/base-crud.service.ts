...
import { BaseQueryParams, ListQueryParams } from '../../models/query-params';
import { ClientResponseError } from '../../models/client-response-error';

export abstract class BaseCrudService<M extends BaseModel> extends BaseService {
	/**
	...
	protected _getFullList<T = M>(
		...
		let request = async (page: number): Promise<Array<any>> => {
			return this._getList(basePath, page, batchSize || 200, queryParams).then(
				(list) => {
	...
	protected _getFirstListItem<T = M>(
		...
		return this._getList<T>(basePath, 1, 1, queryParams).then((result) => {
