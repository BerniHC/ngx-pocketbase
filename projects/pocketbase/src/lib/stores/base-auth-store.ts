import { cookieParse, cookieSerialize, SerializeOptions } from '../utils/cookie';
import { isTokenExpired, getTokenPayload } from '../utils/jwt';
import { Record } from '../models/record';
import { Admin } from '../models/admin';

export type OnStoreChangeFunc = (
	token: string,
...
export abstract class BaseAuthStore {
	...
	save(token: string, model: Record | Admin | null): void {
		...
		if (model !== null && typeof model === 'object') {
			this.baseModel =
				typeof (model as any).collectionId !== 'undefined'
					? new Record(model)
					: new Admin(model);
		} else {
			this.baseModel = null;
